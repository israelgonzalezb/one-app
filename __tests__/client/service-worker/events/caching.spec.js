...
describe('caching', () => {
  ...
  test('caches all app assets and overwrites initial legacy bundle items', async () => {
    ...
    // console.log(caches.snapshot());
    // console.log(JSON.parse(caches.snapshot()['__sw/__meta']['http://localhost/__sw/__meta/__sw/one-app-cache'].body.parts.join('')));
    // console.log(JSON.parse(caches.snapshot()['__sw/__meta']['http://localhost/__sw/__meta/__sw/language-pack-cache'].body.parts.join('')));
  });
